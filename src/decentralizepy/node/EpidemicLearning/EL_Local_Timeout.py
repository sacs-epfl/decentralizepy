    ...
    def run(self):
        ...
        for iteration in range(self.iterations):
            ...
            for neighbor in neighbors_this_round:
                logging.info("Sending to neighbor: %d", neighbor)
            ...
            while True:
                ...
                if cur_time - init_time > self.timeout:
                    break

            if at_least_one_response:
                averaging_deque = dict()
                for neighbor in self.peer_deques:
                    if len(self.peer_deques[neighbor]):
                        averaging_deque[neighbor] = self.peer_deques[
                            neighbor
                        ]  # passed by reference
